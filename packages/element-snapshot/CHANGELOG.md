...
- 400bb83: Support custom normalizers for Markdown table snapshots
...
- 484af4b: Add support for arbitrary elements with `role="combobox"`
...
- 393dfe2: Add support for elements with `role="tree"` and `role="treeitem"`
...
- a0df8a5: Add support for elements with `role="insertion"`, `role="deletion"`, `role="mark"`, `role="subscript"` and `role="superscript"`

  Snapshot text-level elements with these roles, including their implicit HTML equivalents: `<ins>` (`insertion`), `<del>` (`deletion`), `<mark>` (`mark`), `<sub>` (`subscript`) and `<sup>` (`superscript`).
